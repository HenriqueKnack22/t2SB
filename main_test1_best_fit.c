/*
 * main_test1_best_fit.c — TESTE 1: Funcionalidades Básicas (BEST FIT)
 *
 * Objetivo: verificar mymemory_alloc, mymemory_free, mymemory_display,
 *           mymemory_stats e mymemory_cleanup com a estratégia Best Fit.
 *
 * Compile:
 *   gcc -o test1_best_fit  main_test1_best_fit.c  mymemory.c
 */
...
int main(void)
{
    ...
    printf("\n");
    printf("╔══════════════════════════════════════════════════════════╗\n");
    printf("║      TESTE 1 — Funcionalidades Básicas  [BEST FIT]      ║\n");
    printf("╚══════════════════════════════════════════════════════════╝\n\n");

    /* ── 1. Inicialização ─────────────────────────────────────── */
    printf("[ 1 ] Inicialização do pool (2048 bytes)\n");
    mymemory_t *mem = mymemory_init(2048, STRATEGY_BEST_FIT);
    ...
    CHECK(mem->strategy == STRATEGY_BEST_FIT,      "estratégia == BEST FIT");
     ...
     * Best Fit para alloc(20): deve escolher o buraco de 25 (menor
     * que comporta 20), e NÃO o buraco de 50 nem a cauda do pool. */
    ...
    /* Best Fit: 20 bytes cabe em 25 (sobra 5) ou em 50 (sobra 30).
     * A menor sobra é no buraco de 25 → offset esperado = 10. */
    CHECK((char *)p20 - pool_start == 10,
          "BEST FIT: alloc(20) escolheu o menor buraco adequado (offset 10)");
