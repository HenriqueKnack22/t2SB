/*
 * main_test1_worst_fit.c — TESTE 1: Funcionalidades Básicas (WORST FIT)
 *
 * Objetivo: verificar mymemory_alloc, mymemory_free, mymemory_display,
 *           mymemory_stats e mymemory_cleanup com a estratégia Worst Fit.
 *
 * Compile:
 *   gcc -o test1_worst_fit  main_test1_worst_fit.c  mymemory.c
 */
...
int main(void)
{
    ...
    printf("\n");
    printf("╔══════════════════════════════════════════════════════════╗\n");
    printf("║     TESTE 1 — Funcionalidades Básicas  [WORST FIT]      ║\n");
    printf("╚══════════════════════════════════════════════════════════╝\n\n");

    /* ── 1. Inicialização ─────────────────────────────────────── */
    printf("[ 1 ] Inicialização do pool (2048 bytes)\n");
    mymemory_t *mem = mymemory_init(2048, STRATEGY_WORST_FIT);
    ...
    CHECK(mem->strategy == STRATEGY_WORST_FIT,      "estratégia == WORST FIT");
    ...
    /* Cria dois buracos: 25 bytes (offset 10) e 50 bytes (offset 35).
     * A cauda do pool (offset 185 em diante) tem 1863 bytes.
     * Worst Fit para alloc(20): deve escolher o maior buraco disponível,
     * que é a cauda — e NÃO o buraco de 25 nem o de 50. */
    mymemory_free(mem, p25);   /* buraco de 25 no offset 10 */
    mymemory_free(mem, p50);   /* buraco de 50 no offset 35 */
    CHECK(1, "free(p25) e free(p50) sem crash");

    void *p20 = mymemory_alloc(mem, 20);
    CHECK(p20 != NULL, "alloc(20) após dois frees OK");

    /* Worst Fit: escolhe o MAIOR buraco disponível (cauda de 1863 B).
     * Logo p20 deve estar no offset 185 (logo após p100). */
    CHECK((char *)p20 - pool_start == 185,
          "WORST FIT: alloc(20) escolheu o maior buraco (cauda, offset 185)");
